...
class CustomDataset(Dataset):
    ...
    def __init__(self, dataset, split):
        ...
        if dataset == 'coco-lt':
            ...
            self.data_source = ImageList(root='../data/nuswide/',
                                        list_file='../data/nuswide/nw_lt_%s.txt' % split,
                                        label_file='../data/nuswide/nw_labels.txt',
                                        nb_classes=81,
                                        split=split)
...
def build_dataset1(dataset, split):
    ...
    assert dataset in ["coco-lt", "voc-lt", "nus-wide"]

    if split == 'val':
        split = 'test'

    dataset = CustomDataset(dataset=dataset, split=split)
    return dataset
